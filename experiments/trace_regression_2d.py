...
file_path = "/scratch/mmahaut/data/intertva/regression_output/tfMRI/reproducibility/tfmri10-rsfmri10_2d"
...
if __name__ == "__main__":
    ...
    for train_index, test_index in kf.split(idx):
        ...
        np.save(os.path.join(file_path, file), beta)
        # Estimate the results
        ...
        np.save(os.path.join(file_path, file), r2_score(YT, results[idx[test_index]]))
...
file = "mean_mse.npy"
np.save(os.path.join(file_path, file), np.mean([mse]))
print("mean r squared {}".format(np.mean([rsquared])))
file = "mean_rsquared.npy"
np.save(os.path.join(file_path, file), np.mean([rsquared]))
print(results)
print("Mean Error = {}".format(np.linalg.norm(results - Y) ** 0.2 / Y.shape[0]))
print("MSE = {}".format(mean_squared_error(Y, results)))
file = "mse.npy"
np.save(os.path.join(file_path, file), mean_squared_error(Y, results))
file = "r2_score.npy"
np.save(os.path.join(file_path, file), r2_score(Y, results))
